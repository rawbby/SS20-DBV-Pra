...
class Control_window:
    ...
    def setupControl(self, ui: Ui_window, window: QMainWindow):
        ...
        def on_embed(checked):
            ...
            np_foreign = ui.scribble.get_np_image()

            np_foreign = np.dstack((np_foreign, np_mask))
            ui.merge.set_np_foreign(np_foreign)

        def on_smart_fullfill(checked):
            image = ui.scribble.get_np_image()
            keep = ui.scribble.canvas_primary_mask()
            dump = ui.scribble.canvas_secondary_mask()

            output = nn_segmentation_from_masks(image, patch_size, keep, dump)
            ui.scribble.canvas_fill_primary_mask(output)

        def on_open_scribble(checked):
            fwd_open_file_path(ui.scribble.load_image_from_path, window)
        ...
        def on_open_merge(checked):
            fwd_open_file_path(ui.merge.load_image_from_path, window)
        ...
        ui.smart_fullfill.triggered.connect(on_smart_fullfill)
